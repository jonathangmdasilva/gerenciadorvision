 ...
 * @author Jonathan
 */
public class AlterarContasPagar extends javax.swing.JDialog {

    Validar validar = new Validar();
    ContasPagarModel  pagar = new ContasPagarModel();
    
    public AlterarContasPagar(java.awt.Frame parent, boolean modal,ContasPagarModel pag) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        setTitle("Alterar Contas a Pagar");
    ...
    private void limpar(){
        ...
        validar.limpar(jScrollPane1);
    }
    
    
    public void carregarInfs(ContasPagarModel pagar){
        txtCedente.setText(pagar.getCedente());
        txtNumeroDocumento.setText(pagar.getNumeroConta());
        txtValor.setText(validar.converterMoeda(pagar.getValorConta()));
        txtEmissao.setText(validar.FormatarData(pagar.getEmissaoConta()));
        txtVencimento.setText(validar.FormatarData(pagar.getVencimentoConta()));
        if(pagar.getStatus().equals("ABERTO")){
            cmbStatus.setSelectedIndex(0);
        }else{
            cmbStatus.setSelectedIndex(1);
        }
        txtObservacao.setText(pagar.getObservacaoConta());        
    }
    ...
    private void alterar(){
        try {
            ...
            pagarController.alterarContasPagar(pagar);
            validar.exibirConfirmacaoCadastro("Contas à Pagar");
            this.dispose();      
        }catch(Exception e){
            validar.exibirErroAlterarCadastro("Contas Pagar");
