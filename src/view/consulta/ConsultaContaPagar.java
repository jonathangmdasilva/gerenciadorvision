...
import tablemodels.ContasPagarTableModel;
import view.alterar.AlterarContasPagar;
 ...
 * @author Jonathan
 */
public final class ConsultaContaPagar extends javax.swing.JFrame {

    Validar validar = new Validar();
    ContasPagarTableModel pagarTableModel= new ContasPagarTableModel();             
    ContasPagarModel pagarModel = new ContasPagarModel();
    ContasPagarController cc = new ContasPagarController();
    UsuarioModel usuario;
    
    public ConsultaContaPagar(UsuarioModel usu) {
        initComponents();
        ...
        configurarTabela();
        tabelaPagar.setModel(pagarTableModel);
        setVisible(true);
    }
    
    List<ContasPagarModel> listaContas = new ArrayList<>();

    
    public ContasPagarTableModel getModelo() {
	if(pagarTableModel == null){
            pagarTableModel = new ContasPagarTableModel(listaContas);
	}
	...
	if(tabelaPagar == null){
            ...
            tabelaPagar.setModel(getModelo());
	}
            return tabelaPagar;
    }
    ...
    private void initComponents() {
        ...
        setTitle("Consulta de Contas Pagar");
        ...
        tabelaPagar.setModel(getModelo());
        tabelaPagar.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        tabelaPagar.addFocusListener(new java.awt.event.FocusAdapter() {
            public void focusGained(java.awt.event.FocusEvent evt) {
                tabelaPagarFocusGained(evt);
            }
        });
    ...
    private void jMenuItem2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItem2ActionPerformed
        validar.exibirAjudaConsulta("Contas a Pagar");        
        ...
        if(evt.getButton() == MouseEvent.BUTTON1){
            if (evt.getClickCount() == 2) {
                if (usuario.getNivel().equals("ADMINISTRADOR")) {
                    pagarModel = pagarTableModel.getContasPagar(tabelaPagar.getSelectedRow());
                    AlterarContasPagar acp = new AlterarContasPagar(this, rootPaneCheckingEnabled,pagarModel);
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
      try {
            ...
            JOptionPane.showMessageDialog(this, "Erro ao consultar(ABRIR FORM):\n" +ex.getMessage(),"Erro",JOptionPane.ERROR_MESSAGE);
        }
        ...
        switch (cmbParametro.getSelectedIndex()) {
            case 2:
                ...
                while (true) {
                    ...
                    Logger.getLogger(ConsultaContaPagar.class.getName()).log(Level.SEVERE, null, ex);
                }       
                break;
            case 4:
                // TRABALHANDO COM DATA DE VENCIMENTO
                btnConfirmar.setEnabled(false);
                txtChave.setEnabled(false);
                Date inicial,fim;
                REPETIR_DATA:
    ...
    private void tabelaPagarFocusGained(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_tabelaPagarFocusGained
        try {
            int num = cmbParametro.getSelectedIndex();
            String parametro = txtChave.getText();              
            lista(num, parametro);
    ...
    }//GEN-LAST:event_tabelaPagarFocusGained

    public void configurarTabela(){
        tabelaPagar.getColumnModel().getColumn(0).setPreferredWidth(60);
        tabelaPagar.getColumnModel().getColumn(1).setPreferredWidth(350);
        tabelaPagar.getColumnModel().getColumn(2).setPreferredWidth(25);
        tabelaPagar.getColumnModel().getColumn(2).setPreferredWidth(50);
        tabelaPagar.getColumnModel().getColumn(2).setPreferredWidth(75);
    }    
    ...
    public void lista(int num, String param) throws SQLException, ClassNotFoundException{
        switch (num) {
            case 0:
                pagarTableModel.limpar();
                listaContas = cc.listarTodosCedente(param);                
                pagarTableModel.addListaContasPagar(listaContas);
                break;
            case 1:
                pagarTableModel.limpar();
                listaContas = cc.listarCodigo(Integer.parseInt(param));
                pagarTableModel.addListaContasPagar(listaContas);
                break;
            case 2:
                pagarTableModel.limpar();
                listaContas = cc.listarPorStatus(param);
                pagarTableModel.addListaContasPagar(listaContas);
                break;                
            case 3:
                pagarTableModel.limpar();
                listaContas = cc.listarTodosContasPagar();
                pagarTableModel.addListaContasPagar(listaContas);
                break;
            case 4:
                pagarTableModel.limpar();
    ...
    public void listaDatas(Date inicio, Date fim) throws SQLException, ClassNotFoundException{
        pagarTableModel.limpar();                
        listaContas = cc.listarTodasPeriodoVenc(inicio,fim);    
        pagarTableModel.addListaContasPagar(listaContas);
    }
    
    public void excluir(){
        try {            
            int codigo  = listaContas.get(tabelaPagar.getSelectedRow()).getIdConta();
            pagarTableModel.remover(tabelaPagar.getSelectedRow(),codigo);
        } catch (SQLException | ClassNotFoundException ex) {
            Logger.getLogger(ConsultaCliente.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void novo(){
        this.dispose();
        CadastroContasPagar cad= new CadastroContasPagar(this, rootPaneCheckingEnabled,usuario);            
    }
    ...
    private javax.swing.JTable tabelaPagar;
